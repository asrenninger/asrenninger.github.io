...
const globeVisualization = {
    ...
    orbitConfigurations: [],
    satelliteElements: [],
    orbits: [], // To store the path points for calculation
    ...
    generateSatelliteOrbits: function () {
        this.orbitConfigurations = [
            { axis: [0, 1, 0], angle: this.toRad(60), speed: 0.0001, offset: 0 },
            { axis: [1, 1, 0], angle: this.toRad(45), speed: 0.00015, offset: 1000 },
            { axis: [0, 0, 1], angle: this.toRad(30), speed: 0.0002, offset: 2000 },
            { axis: [1, 0, 1], angle: this.toRad(50), speed: 0.00012, offset: 500 },
            { axis: [1, 1, 1], angle: this.toRad(40), speed: 0.00018, offset: 1500 },
        ].slice(0, this.satellites);

        this.orbits = this.orbitConfigurations.map(({ axis, angle }) => {
            return Array.from({ length: 201 }, (_, j) => {
                ...
                // Store raw 3D coordinates for projection during draw/update
                // Use higher resolution or calculate on the fly for smoothness?
                // The original pre-calculated points for drawing the line. 
                // We will just return the lines for drawing here.
    ...
    drawSatellites: function () {
        // Ensure orbits are generated
        if (this.orbits.length === 0) {
            this.generateSatelliteOrbits();
        }
        ...
        this.orbits.forEach((orbit, i) => {
            ...
            this.satelliteElements.push(satellite);
            this.svg.appendChild(satellite);
        });

        // Initial position update to ensure they aren't at 0,0
        this.update(0);
    },

    update: function (time) {
        if (!this.satelliteElements.length) return;

        this.orbitConfigurations.forEach((config, i) => {
            const { axis, angle, speed, offset } = config;

            // Calculate current angle based on time
            // t varies from 0 to 2PI
            const t = ((time + offset) * speed) % (2 * Math.PI);
            ...
            const rotated = this.rotatePoint([x, y, z], axis, angle);
            const [projectedX, projectedY] = this.project(rotated[0] * 120, rotated[1] * 120, rotated[2] * 120);

            this.satelliteElements[i].setAttribute('cx', projectedX);
            this.satelliteElements[i].setAttribute('cy', projectedY);
        });
    },
...
function animate(time) {
    ...
    // Update globe satellites
    globeVisualization.update(time);
...
const flickrSlideshow = {
    // Remove nojsoncallback=1 to get the JSONP response wrapped in jsonFlickrFeed(...)
    feedUrl: "https://api.flickr.com/services/feeds/photos_public.gne?id=199675373@N07&format=json",
    images: [],
    currentIndex: 0,
    interval: 6000,
    imageElement: null,
    ...
    initialize: function () {
        this.imageElement = document.getElementById('flickr-slideshow');
        if (!this.imageElement) return;

        // JSONP Callback needs to be global
        window.jsonFlickrFeed = (data) => {
            if (data && data.items) {
                // Extract image URLs and modify for higher res
                this.images = data.items.map(item => item.media.m.replace('_m.jpg', '_b.jpg'));
    ...
    showImage: function (index) {
        ...
        const img = new Image();
        img.onload = () => {
            // If we have a placeholder (fav.png), we want to respect its aspect ratio or cover behavior
            // The style is already set to object-fit: cover in HTML
        ...
        // Handle error in case _b.jpg doesn't exist (fallback to original if needed?)
        // For simplicity, we assume _b exists (standard for Flickr uploads), but we could add onerror here.
        img.onerror = () => {
            // Fallback to medium if large fails
            console.log("Failed to load large image, trying medium");
            const fallbackUrl = imageUrl.replace('_b.jpg', '_m.jpg');
...
flickrSlideshow.initialize();

// Trigger shuffle layout
setTimeout(() => {
    myShuffle.layout();
}, 500);
